...
exports.addMatch = (req, res) => {
	connection.userType(req.session.user).query('CALL add_match(?, ?, ?, ?)',
		[req.body.timeStart,
		 req.body.timeEnd,
...
exports.addTeamInMatch = (req, res) => {
	...
	connection.userType(req.session.user.type).query(query,
		...
		(err, rows, fields) => {
			if(!err){
				logs.createLog(req.session.user.id, "Added a team to match");
				return res.status(200).send(rows[0]);
			}else{
				return res.status(500).send("Internal server error occurred");
			}
		...
		(err, rows, fields) => {
			...
			if(!err){
				connection.userType('A').query('CALL delete_team_in_match(?,?)', [matchId, teamId] , (err, rows) => {
						logs.createLog(req.session.user.id, "Removed team " + teamId +" from match " + match_id);
						return res.status(200).send(deleted[0]);
					})
			}else{
				return res.status(500).send("Internal server error occurred");
